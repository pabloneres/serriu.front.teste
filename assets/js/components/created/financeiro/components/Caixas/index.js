...
import { index, show, store, destroy } from "~/controllers/controller";
...
function Caixas() {
  ...
  }, [permission, reload, selectedClinic.id, token, user.id]);
  ...
  const handleOpen = () => {
    store(token, `/caixa/abrir/${showModalOpen}`, {
      valor: valorInicial
    })
      .then(_ => {
        setReload(!reload);
  ...
  const handleClose = () => {
    store(token, `/caixa/fechar/${showModalOpen}`, {
      valor: valorInicial
    }).then(_ => {
      setReload(!reload);
      setShowModalOpen(undefined);
  ...
  const send = data => {
    ...
    store(token, `caixa/fechar/${showModalClose}`, obj).then(({ data }) => {
      setReload(!reload);
      setShowModalClose(undefined);
    });
  };
  const handleAddCaixa = () => {
    store(token, "caixa/criar", {
