...
function ModalFinanceiro() {
  ...
  const [pagamentoValue2, setPagamentoValue2] = useState();
  ...
  const [pagamentoDetails, setPagamentoDetails] = useState(undefined);
  ...
  useEffect(() => {
    ...
    return;
  }, [procedimentos, metodoPagamento, selecionado]);
  ...
  const total = Number(procedimentos.reduce((a, b) => a + Number(b.valor), 0));

  const totalDesconto = Number(
    procedimentos.reduce((a, b) => a + Number(b.desconto), 0)
  );

  const totalPago =
    Number(orcamento.valorDesconto) - Number(orcamento.restante);
  ...
  return (
    <Card title="Negociação">
      <Container>
                ...
                <ContainerDashed centered width="100">
                  <FormFixed border={0} block>
                    ...
                    <span>
                      ...
                      {convertMoney(
                        (returnValorSelecionado() *
                          selectedClinic.config.entMinima) /
                          100
                      )}
                    </span>
                  </FormFixed>
        ...
        {pagamentoDetails ? (
          <ContainerSide>
            ...
            {pagamentoValue <= orcamento.valor ? (
              <EspecialidadeContainerAll>
                <EspecialidadeContainer>
                  <Especialidades>
                    {especialidadesSelecionado.map((item, index) => {
                      return (
                        <Especialidade
                          ...
                          // blur={value => setValorDistribuir(valorDistribuir - Number(value))}
                          // valorDistribuir={valorDistribuir}
                          // disabled={item.valorAplicado}
                        />
                      );
                    })}
                  </Especialidades>
                ...
                <>
                  ...
                  {loadingButton ? (
                    ...
                    <Button
                      ...
                      type="primary"
                      block
                      disabled={distribuirAtualizado() !== 0}
                    >
                      Receber
                    </Button>
                  )}
      ...
      <Negociacoes negociacoes={orcamento.negociacoes} />
    </Card>
  );
}
