...
function Negociacao({data, paciente, close, handleAdicionarSaldo}) {
							...
							<ContainerScroll border={1} dashed>
								<Table
									...
									columns={[
										...
										{
											title : "",
											render: data => <Button icon={<DollarCircleOutlined />} type="primary" onClick={() => getNegociacao(data.id)} />
										}
									]}
									dataSource={data}
									expandable={{
			...
			<div className="footer_drawer">
				{faturamento && pagamentoDetails ? (
					<EspecialidadeContainerAll>
						<div className="info">
							<h2>Á Distribuir</h2>
							<span style={{color: distribuirAtualizado() < 0 ? "red" : "green"}}>{convertMoney(distribuirAtualizado())}</span>
						</div>
						<EspecialidadeContainer>
							<Especialidades>
