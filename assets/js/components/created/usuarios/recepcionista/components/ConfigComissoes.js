...
import {
  Input, Form, Card, Radio, Skeleton, Space, Tooltip, Table, Button, Spin
} from 'antd'
...
export function ConfigComissoes() {
  ...
  useEffect(() => {
    index(token, `/comissao_config/${params.id}?clinica_id=${selectedClinic.id}`).then(({ data }) => {
      setConfig(data)
    })
  ...
  const changeValue = (e) => {
    setLoading(true)
    store(token, `comissao_config/${params.id}`, { ...e, clinica_id: selectedClinic.id }).then(data => {
      setReload(!reload)
      setLoading(false)
  ...
  const changeValueNoSend = (e) => {
    setLoading(true)
    store(token, `comissao_config/${params.id}`, { vista, boleto, clinica_id: selectedClinic.id }).then(data => {
      setReload(!reload)
      setLoading(false)
      return Notify('success', 'Sucesso', 'Atualizado com sucesso')
    })
        ...
        <Card title="Configurações de comissão">
          ...
          >
            ...
            <Form.Item label="Forma de recebimento ?" name="forma_recebimento" >
              <Radio.Group
                onChange={e => changeValue({ forma_recebimento: e.target.value })}
            ...
            <Form.Item label="Necessita aprovação ?" name="necessita_aprovacao" >
              <Radio.Group
                onChange={e => changeValue({ necessita_aprovacao: e.target.value })}
                disabled={!config.recebe_comissao}
                options={[
                  {
                    label: 'Sim',
            ...
            <Form.Item label="Executar sem saldo ?" name="executar_sem_saldo" >
              <Radio.Group
                onChange={e => changeValue({ executar_sem_saldo: e.target.value })}
                disabled={!config.recebe_comissao}
                options={[
                  {
                    label: 'Sim',
            ...
            </Form.Item>
            {
              config.funcao === 'executor' ?
                ...
                <>
                  ...
                  <FormRow columns={2} style={{ border: '1px solid', borderColor: '#c4c4c4', padding: 10, borderRadius: '10px' }}>
                    ...
                    </Form.Item>
                    {
                      loading ?
                        <Spin />
                        : <Button type="primary" onClick={e => changeValueNoSend()}>Enviar</Button>}
                  </FormRow>
                </>
            }
