...
function DentistaPage() {
  ...
  const loadDentist = useCallback((search = "", pagination = {}) => {
    setLoading(true);
    index("users", {
  ...
  const columns = [
    {
      title: "ID",
      dataIndex: "id",
      render: data => <Link to={`/dentista/editar/${data}`}>{data}</Link>
    },
    {
      title: "Nome",
      render: data => (
        <Link to={`/dentista/editar/${data.id}`}>
      ...
      render: data => (
        <Space size="middle">
          <Tooltip placement="top" title="Editar">
            <span
              onClick={() => history.push("/dentista/editar/" + data.id)}
