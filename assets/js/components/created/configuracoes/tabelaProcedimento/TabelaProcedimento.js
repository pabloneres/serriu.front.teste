...
function TabelaProcedimento() {
	...
	const handleSubmit = () => {
		...
		if( fields.id )
		{
			update('procedimento', fields.id, fields)
			.then(() => {
				setFields(undefined)
				setReload(!reload)
			})
	...
	useEffect(() => {
		...
		index("procedimento", {id: params.id})
		.then(({data}) => {
			setProcedimentos(data)
		})
		index("laboratorio", {clinic_id: selectedClinic.id}).then(({data}) => {
			setLabs(data)
		})
		setLoading(false)
	}, [show, reload])
	...
	useEffect(() => {
		if( labId )
		{
			index("laboratorioServicos", {laboratorio_id: labId}).then(({data}) => {
				setServices(data)
			})
		}
	}, [labId])
